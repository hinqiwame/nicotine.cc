...
void Aimbot::knife( ) {
	...
	if( target.record ) {
		// set target tick.
		core.m_cmd->m_tick = game::TIME_TO_TICKS( target.record->m_pred_time + core.m_lerp );

		// set view angles.
		core.m_cmd->m_view_angles = target.angle;

		// set attack1 or attack2.
		core.m_cmd->m_buttons |= target.stab ? IN_ATTACK2 : IN_ATTACK;
...
bool Aimbot::CanKnife( LagRecord* record, ang_t angle, bool& stab ) {
	...
	bool first = core.m_weapon->m_flNextPrimaryAttack( ) + 0.4f < g_csgo.m_globals->m_curtime;
...
bool Aimbot::KnifeIsBehind( LagRecord* record ) {
	vec3_t delta{ record->m_origin - core.m_shoot_pos };
	delta.z = 0.f;
	delta.normalize( );
