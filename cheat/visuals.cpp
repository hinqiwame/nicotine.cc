...
void Visuals::StatusIndicators() {
	...
	if (g_menu.main.visuals.indicators.get(1) && ((core.m_buttons & IN_JUMP) || !(core.m_flags & FL_ONGROUND)) && core.m_lagcomp) {
		Indicator_t ind{ };
		ind.color = Color(150, 200, 60, 255);
		ind.text = XOR("LC");
	...
	if (g_menu.main.visuals.indicators.get(0)) {
		...
		Indicator_t ind{ };
		ind.color = change > 35.f ? Color(150, 200, 60, 255) : Color(255, 0, 0, 255);
		ind.text = XOR("LBY");
		indicators.push_back(ind);
	}
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string((core.m_width / 2), (core.m_height / 2) + 65 + (11 * i), indicator.color, indicator.text, render::ALIGN_CENTER);
	}
...
void Visuals::ManualAntiAim() {
	...
	auto left_pos = vec2_t(width / 2 - 45, height / 2), right_pos = vec2_t(width / 2 + 45, height / 2), down_pos = vec2_t(width / 2, height / 2 + 45), siz = vec2_t(8, 8);
	...
	if (!filled[0]) {
		...
		for (unsigned int p = 0; p < vertices_left.size(); p++) {
			render::rotate_point(vertices_left[p].m_pos, left_pos, false, 3.15);
		}
		filled[0] = true;
	}
	...
	if (!filled[1]) {
		...
		for (unsigned int p = 0; p < vertices_right.size(); p++) {
			render::rotate_point(vertices_right[p].m_pos, right_pos, false, 0);
		}
		filled[1] = true;
	}
	...
	if (!filled[2]) {
		...
		for (unsigned int p = 0; p < vertices_down.size(); p++) {
			render::rotate_point(vertices_down[p].m_pos, down_pos, false, 61.25);
		}
		filled[2] = true;
	}

	render::textured_polygon(vertices_left.size(), vertices_left, Color(255, 255, 255, 30));
	render::textured_polygon(vertices_right.size(), vertices_right, Color(255, 255, 255, 30));
	render::textured_polygon(vertices_down.size(), vertices_down, Color(255, 255, 255, 30));

	if (g_hvh.m_left)
		render::textured_polygon(vertices_left.size(), vertices_left, Color(g_menu.main.antiaim.color_manul_antiaim.get()));

	if (g_hvh.m_right)
		render::textured_polygon(vertices_right.size(), vertices_right, Color(g_menu.main.antiaim.color_manul_antiaim.get()));

	if (g_hvh.m_back)
		render::textured_polygon(vertices_down.size(), vertices_down, Color(g_menu.main.antiaim.color_manul_antiaim.get()));
}
		...
		switch (g_menu.main.players.health_style.get() % 2) {
			...
			case 1: {
				...
				if (dormant)
					...
					render::gradient(box.x - 5, y + h - fill - 1, 2, fill + 2, {g_menu.main.players.health_1.get()}, { g_menu.main.players.health_2.get() });
			} break;
		}
